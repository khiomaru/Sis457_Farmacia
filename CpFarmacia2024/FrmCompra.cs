...
namespace CpFarmacia2024
{
    ...
    public partial class FrmCompra : Form
    {
        ...
        private void FrmCompra_Load(object sender, EventArgs e)
        {
            ...
            dgvCompras.Columns.Add("codigo", "C�digo");
         ...
         decimal.TryParse(txtPrecioVenta.Text, out decimal precioVenta))
            {
                if (precioCompra > precioVenta)
                {
                    ...
                    MessageBox.Show("El precio de venta debe ser mayor que el precio de compra.", "Error de Validaci�n", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        private void ibtnAgregar_Click(object sender, EventArgs e)
        {
            if (!validar())
            {
                // Si no es v�lido, muestra un mensaje y no contin�a
                MessageBox.Show("Por favor, corrige los errores antes de agregar el Medicamento.", "Error de Validaci�n", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void iBtnRegistrar_Click(object sender, EventArgs e)
        {
            ...
            // Verifica si el n�mero de factura ya existe
            if (CompraCln.ExisteNumeroFactura(numeroDocumento))
            {
                MessageBox.Show("El n�mero de factura ya existe. Por favor, ingrese un n�mero �nico.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            //// Verifica que el tipo de documento seleccionado sea v�lido
            if (cbxTipoDocumento.SelectedItem == null || !cbxTipoDocumento.Items.Contains(cbxTipoDocumento.SelectedItem))
            {
                MessageBox.Show("Por favor, seleccione un tipo de documento v�lido.", "Error de Validaci�n", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (compra == null)
            {
                return; // Se gener� un error en la creaci�n de la compra
            }
            ...
            try
            {
                ...
                MessageBox.Show("Compra registrada correctamente.", "�xito", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
        ...
        private void RegistrarDetallesCompra(int idCompraRegistrada)
        {
            foreach (DataGridViewRow row in dgvCompras.Rows)
            {
                if (row.Cells["IdMedicamento"].Value != null)
                {
                    ...
                    {
                        return; // Si hay un error de validaci�n, detenemos la ejecuci�n
                    }

                    // Aqu� es donde debes inicializar detalleCompra
        ...
        private bool ValidarDetallesCompra(DataGridViewRow row, int idCompraRegistrada, out DetalleCompra detalleCompra)
        {
            ...
            // Validar que los precios y la cantidad sean v�lidos
            if (precioCompra < 0 || precioVenta < 0 || cantidad <= 0)
            {
                MessageBox.Show("Los valores de precio y cantidad deben ser positivos.", "Error de Validaci�n", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void ManejarErrorValidacion(DbEntityValidationException validationEx, string mensaje)
        {
            ...
            MessageBox.Show(errorDetails.ToString(), "Error de Validaci�n", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        ...
        private void ActualizarStockYPrecios()
        {
            foreach (DataGridViewRow row in dgvCompras.Rows)
            {
                if (row.Cells["IdMedicamento"].Value != null)
                {
                    ...
                    try
                    {
                        // Llama al m�todo ActualizarStock con los par�metros correctos
        ...
        private void iBtnQuitar_Click(object sender, EventArgs e)
        {
            ...
            if (dgvCompras.SelectedRows.Count > 0)
            {
                ...
                // Calcular el nuevo total a pagar despu�s de eliminar el Medicamento
                CalcularTotalPagar();
            }
            else
            {
